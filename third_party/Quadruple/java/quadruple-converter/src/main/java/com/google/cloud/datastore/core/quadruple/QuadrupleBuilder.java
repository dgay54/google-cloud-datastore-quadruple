...
public class QuadrupleBuilder {
  ...
  private void findBinaryMantissa(int exp10, double exp2, long[] mantissa) {
    ...
    System.err.printf(
        "before roundUp: %08x %08x %08x %08x %08x %08x\n",
        product[0], product[1], product[2], product[3], product[4], product[5]);
    boolean exact = product[4] == 0 && product[5] == 0;
    long prev3 = product[3];
    exp2 += roundUp(product); // round up, may require exponent correction
    System.err.printf(
        "after roundUp: %08x %08x %08x %08x %08x %08x\n",
        product[0], product[1], product[2], product[3], product[4], product[5]);
    ...
    if (((long) (exp2)) >= EXPONENT_OF_INFINITY) {
      ...
      this.rounding = exact ? 0 : prev3 == product[3] ? 1 : -1;
    }
  }
